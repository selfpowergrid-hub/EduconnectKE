...
const Fees = () => {
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: "#fff", border: "1px solid #E8EAF0", borderRadius: 12, overflow: "hidden", flex: 1 }}>
        {activeTab === "balances" ? (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: 13 }}>
              <thead style={{ background: "#FAFBFC", borderBottom: "1px solid #E8EAF0" }}>
                <tr>
                  {["ADM No.", "Student Name", "Total Billed", "Total Paid", "Balance", "Action"].map(h => (
              ...
              <tbody>
                {filteredStudents.slice(0, 50).map((s, idx) => (
                  <tr key={s.id} style={{ borderBottom: "1px solid #F7F8FA", background: idx % 2 === 0 ? "#fff" : "#FAFBFC" }}>
                    ...
                    <td style={{ padding: "12px 18px" }}>KES {s.feeTotal.toLocaleString()}</td>
                    <td style={{ padding: "12px 18px", color: "#1B6B3A", fontWeight: 600 }}>KES {s.feePaid.toLocaleString()}</td>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: 13 }}>
              <thead style={{ background: "#FAFBFC", borderBottom: "1px solid #E8EAF0" }}>
                <tr>
                  {["Trans. ID", "Date", "Student/Payer", "Amount", "Phone", "Status"].map(h => (
                    <th key={h} style={{ padding: "12px 18px", fontWeight: 700, color: "#8A8FA8", fontSize: 11, textTransform: "uppercase" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filteredTransactions.map((t, idx) => (
                  <tr key={t.id} style={{ borderBottom: "1px solid #F7F8FA", background: idx % 2 === 0 ? "#fff" : "#FAFBFC" }}>
                    ...
                    <td style={{ padding: "12px 18px", color: "#8A8FA8" }}>{t.date}</td>
                    <td style={{ padding: "12px 18px" }}>
                      <div style={{ fontWeight: 750, color: "#1A1A2E" }}>{t.name}</div>
                    ...
                    <td style={{ padding: "12px 18px", color: "#4A4A6A" }}>{t.phone}</td>
                    <td style={{ padding: "12px 18px" }}>
                      <span style={{ 
                        padding: "2px 8px", 
                        borderRadius: 10, 
